...
# USERNAME=go-backend-template

# Prompt to enter a password for the PostgreSQL go-backend-template user (rather than hard-coding
# a password in this script).
read -p 'Enter password for go-backend-template DB user: ' DB_PASSWORD
...
# Set up the go-backend-template DB and create a user account with the password entered earlier.
psql -c "CREATE DATABASE go-backend-template"
psql -d go-backend-template -c "CREATE EXTENSION IF NOT EXISTS citext"
psql -d go-backend-template -c "CREATE ROLE go-backend-template WITH LOGIN PASSWORD '${DB_PASSWORD}'"

# Add a DSN for connecting to the go-backend-template database to the system-wide environment
# variables in the /etc/environment file.
echo "GO_BACKEND_TEMPLATE_DB_DSN=postgres://go-backend-template:${DB_PASSWORD}@localhost/go-backend-template" | sudo tee -a /etc/environment
