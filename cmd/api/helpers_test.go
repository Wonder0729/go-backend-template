...
$ go test -v -run ^TestReadString$ github.com/wonder0729/go-backend-template/cmd/api

As a side note, you can use the -failfast flag to stop the tests running after
the first failure, if you want, like so:

$ go test -failfast -v -run ^TestReadString$ github.com/wonder0729/go-backend-templateackend-templateackend-templateackend-template/cmd/api
